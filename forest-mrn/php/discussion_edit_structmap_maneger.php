<?php
	...
	$result_struct_start_time = $mysqli->query("SELECT MAX(start_time) FROM network_sturuct_activity WHERE user_id = '$user_id' AND map_id = '$map_id' ORDER BY start_time DESC");
	...
	if($purpose === 'record'){
		...
		if($record_thing === 'node'){
			...
			$mysqli->query("INSERT INTO network_nodes_activity (user_id, map_id, node_id, label, node_x, node_y, node_type, time, updated_time)
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$mysqli->query("INSERT INTO network_edges_activity (user_id, map_id, edge_start, edge_end, time)
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$mysqli->query($sql = "INSERT INTO network_mindmap_connect(user_id, map_id, network_node_id, mindmap_node_id, time)
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$mysqli->query("INSERT INTO network_ontology_activity (user_id, map_id, ontology_id, node_id, time)
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$mysqli->query("INSERT INTO network_recruit (user_id, map_id, node_id, ontology_id, result_recruit, time)
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$mysqli->query("INSERT INTO network_edges_activity (user_id, map_id, edge_start, edge_end, edge_label, time)
		...
		if($update_thing === 'node'){
			...
			if($select_update === 'point'){
				...
				$mysqli->query("UPDATE network_nodes_activity SET node_x = '$node_update_thing1', node_y = '$node_update_thing2', updated_time = '$timestamp' 
				                WHERE user_id = '$user_id' AND map_id = '$map_id' AND node_id = '$node_id' AND updated_time >= '$struct_start_time'");
		...
		if($delete_thing === 'node'){
			$node_id = $_POST["node_id"];
			$mysqli->query("DELETE FROM network_nodes_activity WHERE user_id = '$user_id' AND map_id = '$map_id' AND node_id = '$node_id' AND updated_time >= '$struct_start_time'");
		}else if($delete_thing === 'edge'){
			$edge_start = $_POST["edge_start"];          //エッジ開始
			$edge_end = $_POST["edge_end"]; 
			if($edge_start === ""){
				$mysqli->query("DELETE FROM network_edges_activity WHERE user_id = '$user_id' AND map_id = '$map_id' AND edge_end = '$edge_end' AND time >= '$struct_start_time'");
			}else if($edge_end === ""){
				$mysqli->query("DELETE FROM network_edges_activity WHERE user_id = '$user_id' AND map_id = '$map_id' AND edge_start = '$edge_start' AND time >= '$struct_start_time'");
			}else{
				$mysqli->query("DELETE FROM network_edges_activity WHERE user_id = '$user_id' AND map_id = '$map_id' AND edge_start = '$edge_start' AND edge_end = '$edge_end' AND time >= '$struct_start_time'");
			}
