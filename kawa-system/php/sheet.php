<?php
	...
	function showSheet(){
		...
		if($result = $mysqli->query($sql)){
			while($row = mysqli_fetch_assoc($result)){
				echo"<p><label><input type='radio' name='map' value='".$row['node_id']."'>"  .$row['updated_at'].  "  "  .$row['name'].  "</label></p>";
			}
...
function show_user(){
	...
	if($result == TRUE){
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			echo"<option name='user' value='".$row['map_id']."'>"  .$row['updated_at'].  "  "  .$user_name.  "</option>";
