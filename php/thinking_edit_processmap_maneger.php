<?php
	...
	if($purpose === 'record'){
		...
		if($record_thing === 'node'){
			...
			$mysqli->query("INSERT INTO network_nodes_activity (user_id, sheet_id, node_id, label, node_x, node_y, node_type, time, updated_time, mindmap_node_id)
			                VALUES ('$user_id', '$sheet_id', '$node_id', '$label', '$x', '$y', '$node_type', '$timestamp',  '$timestamp', '$selected_node_id')");
			...
			$timestamp = date("Y-m-d H:i:s") . "." . substr(explode(".", (microtime(true) . ""))[1], 0, 3);
			$selected_node_id = $_POST["selected_node_id"];
			$mysqli->query("INSERT INTO network_edges_activity (user_id, sheet_id, edge_start, edge_end, time, mindmap_node_id)
			                VALUES ('$user_id', '$sheet_id', '$edge_start', '$edge_end', '$timestamp', '$selected_node_id')");
			...
			$mysqli->query("INSERT INTO network_edges_activity (user_id, sheet_id, edge_start, edge_end, edge_label, time)
			                VALUES ('$user_id', '$sheet_id', '$edge_start', '$edge_end', '$edge_label', '$timestamp', '".$selected_node_id."')");
		...
		if($update_thing === 'node'){
			...
			if($select_update === 'point'){
				...
				WHERE user_id = '$user_id' AND sheet_id = '$sheet_id' AND node_id = '$node_id'");
				if($mysqli->error){
					echo "Error update label: " . $mysqli->error;
				}
			}
		}
