...
if($purpose === "select_meeting_utterance") {
    ...
    $result_discussionmap_node = $mysqli->query("SELECT node_id, label, node_x, node_y, node_type FROM network_nodes_activity
            WHERE user_id = '$user_id' AND sheet_id = '$sheet_id'AND mindmap_node_id = '".$selected_node_id."' AND updated_time >= '$target_map_created_start_times'
    ...
    $result_discussionmap_edge = $mysqli->query("SELECT edge_start, edge_end, edge_label FROM network_edges_activity
              WHERE user_id = '$user_id' AND sheet_id = '$sheet_id'AND mindmap_node_id = '".$selected_node_id."' AND time >= '$target_map_created_start_times'
              ORDER BY time DESC ");
    $discussionmap_edge = [];
    ...
    */
    ...
    $result_t_candidate = $mysqli->query("SELECT id, activity_type, concept_id, content, appeared_at, trigger_on FROM trigger_candidates
            WHERE sheet_id = '$sheet_id' ORDER BY appeared_at DESC");
    $t_candidate = [];
    $t_candidate_concept = [];
    if (!$result_t_candidate) {
        echo "SELECT id, activity_type, concept_id, content, appeared_at, trigger_on FROM trigger_candidatesWHERE sheet_id = '$sheet_id' ORDER BY appeared_at DESC";
        return;
    }
    ...
    // ノードのバージョン情報を取得
    $result_node_versions = $mysqli->query("SELECT id, parent_node_id, appeared_at, disappeared_at, content FROM node_versions WHERE node_id = '".$selected_node_id."'ORDER BY appeared_at ASC");
    $node_versions = [];
    ...
    $result_trigger = $mysqli->query("SELECT * FROM triggers
                            WHERE node_version_from IN (SELECT id FROM node_versions WHERE node_id = '".$selected_node_id."') AND deleted = 0");
    $trigger = [];
